...
**개인 맞춤형 주식 상담 서비스**를 제공하는 AI 봇입니다. Claude Sonnet 4.5 기반으로 사용자와 자연스러운 대화를 나누며, 보유 종목에 대한 평가와 투자 조언을 제공합니다.

### ✨ 주요 특징

#### 🎨 스타일 적응형 응답
사용자가 원하는 톤과 스타일로 자유롭게 응답합니다:
- **"친구같이"**: 편하고 친근한 말투
- **"전문가처럼"**: 데이터 중심의 격식있는 분석
- **"직설적으로"**: 솔직하고 거침없는 평가
- **"취한 친구처럼"**: 감정적이고 과장된 표현
- **기타 다양한 스타일**: 사용자가 요청하는 어떤 스타일이든 적응 가능

#### 💬 24시간 대화 컨텍스트 유지
- 이전 대화 내용을 기억하여 연속적인 대화 가능
- AI 응답에 답장(Reply)하여 추가 질문 가능
- 자동 만료 처리로 효율적인 리소스 관리

#### 📊 실시간 데이터 기반 분석
- 최신 주가 및 거래량 데이터 조회
- 투자자별 거래 동향 분석 (기관/외국인/개인)
- Perplexity 검색을 통한 최신 뉴스 및 이슈 파악

### 🎯 사용 가능한 명령어

#### `/evaluate` - 개인화된 종목 평가
보유 종목에 대한 맞춤형 평가를 받을 수 있습니다.

**사용 방법:**
```
1. /evaluate 입력
2. 종목 코드 또는 이름 입력 (예: 005930 또는 삼성전자)
3. 평균 매수가 입력 (예: 70000)
4. 보유 기간 입력 (예: 6개월)
5. 원하는 피드백 스타일 입력 (예: 친구같이)
6. 매매 배경 입력 (선택사항)
```
...
#### `/report` - 상세 분석 보고서
12개 AI 에이전트가 협업하여 작성한 전문가급 분석 보고서를 받을 수 있습니다.

**사용 방법:**
```
1. /report 입력
2. 종목 코드 또는 이름 입력
3. 대기 (5-10분 소요, 요청이 많을 경우 더 길어질 수 있음)
4. HTML 파일로 보고서 전송
```

**특징:**
- GPT-4.1 기반 12개 전문 에이전트 협업
- 기술적/재무적/산업 분석 종합
- 다운로드 가능한 HTML 형식
- 24시간 캐시로 빠른 재조회

#### `/history` - 분석 히스토리
특정 종목에 대한 과거 분석 기록을 확인할 수 있습니다.
...
이 봇은 [PRISM-INSIGHT 공식 텔레그램 채널](https://t.me/stock_ai_agent) 구독자만 사용할 수 있습니다.

**채널에서 제공하는 서비스:**
- 장 시작/마감 시 AI 선별 특징주 3개 소개
- 상세 분석 보고서 자동 전송
- 매매 시뮬레이션 실시간 알림
- 자동매매 실적 리포트
...
**예시 1: 친구같이 말해주는 봇**
```
사용자: "삼성전자 70,000원에 샀는데 어떤가요?"
스타일: "친구같이"

봇: "야 삼성전자 지금 72,000원인데? 👍 2.8% 올랐네!
    요즘 HBM 수요 때문에 떡상 중이야. 외국인들도 쓸어담고 있고.
    근데 단기로 먹으려면 75,000원쯤에서 익절 생각해봐.
    장기로 가려면 그냥 묻어두는 것도 나쁘지 않아 보여 🚀"
```
...
봇: "현재 삼성전자는 72,000원에 거래되고 있어 +2.8% 수익 상태입니다.

    기술적 분석:
    - 20일 이동평균선(70,500원) 상단 돌파
    - RSI 62로 과열 구간 진입 전
    - 거래량 증가 추세 (+15%)
    ...
    종합 의견: 단기적으로 75,000원 목표가 도달 시
    부분 익절을 권장드리며, 장기 보유 시 HBM 수요
    증가에 따른 추가 상승 여력이 있습니다."
```

### 🛠️ 기술 스택

- **AI 모델**: Anthropic Claude Sonnet 4.5
- **데이터**: MCP Servers (kospi_kosdaq, perplexity, time)
...
- **대화 컨텍스트 캐싱**: 24시간 메모리 기반 저장
- **비동기 처리**: asyncio 기반 효율적 리소스 관리
- **보고서 캐싱**: 24시간 이내 중복 요청 즉시 응답
